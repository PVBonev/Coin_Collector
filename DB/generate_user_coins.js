...
USER_IDS.forEach((userId) => {
  ...
  for (let i = 0; i < coinCount; i++) {
    ...
    values.push(
      `(${userId}, ${catalogId}, '${grade}', '${status}', ${price}, ${buyPrice}, '${buyDate}', '${location}', '${safeNote}')`,
    );
  }
});
...
try {
  ...
  console.log(
    `Success! Generated ${values.length} coins with detailed financial data.`,
  );
